Feature: Work status service
  ...
  Scenario: New evidence starts as unverified digital evidence
    Given no user-confirmed status exists for evidence "status-cucumber-1"
  ...
  Scenario: User confirmation becomes the current work status
    When the user marks evidence "status-cucumber-2" as "RESOLVED" with reason "Clarified verbally."
